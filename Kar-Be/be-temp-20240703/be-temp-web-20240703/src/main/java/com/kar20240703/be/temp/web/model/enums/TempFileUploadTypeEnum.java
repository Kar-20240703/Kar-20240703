...
import com.kar20240703.be.temp.web.util.MyFileTypeUtil;
...
@AllArgsConstructor
@Getter
public enum TempFileUploadTypeEnum {
    ...
    public String checkFileType(MultipartFile file) {
        ...
        return TempFileUploadTypeEnum.checkFileType(this, file);
    ...
    public void checkFileSize(MultipartFile file) {
        ...
        TempFileUploadTypeEnum.checkFileSize(this, file);
    ...
    @SneakyThrows
    @Nullable
    public static String checkFileType(TempFileUploadTypeEnum tempFileUploadTypeEnum, MultipartFile file) {
        ...
        // 获取：支持上传的文件类型
        Set<String> acceptFileTypeSet = tempFileUploadTypeEnum.getAcceptFileTypeSet();
     ...
     * 检查：文件大小
     */
    public static void checkFileSize(TempFileUploadTypeEnum tempFileUploadTypeEnum, MultipartFile file) {

        long maxFileSize = tempFileUploadTypeEnum.getMaxFileSize();
     ...
     */
    @NotNull
    public static String uploadCheckWillError(MultipartFile file, TempFileUploadTypeEnum tempFileUploadTypeEnum) {

        Assert.notNull(file, "file 不能为空");
        Assert.notNull(tempFileUploadTypeEnum, "uploadType 不能为空");

        tempFileUploadTypeEnum.checkFileSize(file); // 检查：文件大小
        ...
        String fileType = tempFileUploadTypeEnum.checkFileType(file);
