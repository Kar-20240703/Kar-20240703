package com.kar20240703.be.temp.generate.util;
...
import com.kar20240703.be.temp.generate.bo.BeApi;
import com.kar20240703.be.temp.generate.bo.BeApiParameter;
import com.kar20240703.be.temp.generate.bo.BeApiSchema;
import com.kar20240703.be.temp.generate.interfaces.BeApiField;
...
public class SpringDocUtil {
    ...
    private static String SPRING_DOC_ENDPOINT = "http://127.0.0.1:8002/v3/api-docs/be-base-web-20240703";
    ...
    public static HashMap<String, HashMap<String, BeApi>> get(String springDocEndpoint) {
        ...
        HashMap<String, BeApiSchema> beApiSchemaMap = MapUtil.newHashMap(schemas.size()); // 所有的对象，key是对象的名称

        List<BeApiSchema> todoHandleBeApiSchemaList = new ArrayList<>(); // 待处理的：对象类型集合
    ...
    private static void handleResult(JSONObject paths, HashMap<String, HashMap<String, BeApi>> result,
        HashMap<String, BeApiSchema> beApiSchemaMap) {
     ...
     * 处理：responses
     */
    private static void handleResultResponses(HashMap<String, BeApiSchema> beApiSchemaMap,
        ...
        BeApiSchema beApiSchema = new BeApiSchema();
        ...
        BeApiSchema propertiesBeApiSchema = beApiSchemaMap.get(beApiSchemaMapKey); // 从 map中获取对象
        ...
        HashMap<String, BeApiField> propertiesFieldMap = MapUtil.newHashMap();
     ...
     * 处理：parameters
     */
    private static void handleResultParameters(HashMap<String, BeApiSchema> beApiSchemaMap,
        Map.Entry<String, Object> item, BeApi beApi, JSONArray parameters) {

        Map<String, BeApiField> parameterMap = MapUtil.newHashMap();
     ...
     * 设置：一般类型到 map里
     */
    private static void handleResultParametersParameter(Map<String, BeApiField> parameterMap, JSONObject parameter,
        String name) {

        BeApiParameter beApiParameter = new BeApiParameter();
     ...
     * 处理：beApiParameter的属性
     */
    private static void handleBeApiParameter(JSONObject jsonObject, BeApiParameter beApiParameter) {
     ...
     * 处理：requestBody
     */
    private static void handleResultRequestBody(HashMap<String, BeApiSchema> beApiSchemaMap,
        ...
        BeApiSchema beApiSchema = beApiSchemaMap.get(beApiSchemaMapKey); // 从 map中获取对象
     ...
     * 处理：待处理的：对象类型集合
     */
    private static void handleTodoHandleBeApiSchemaList(HashMap<String, BeApiSchema> beApiSchemaMap,
        List<BeApiSchema> todoHandleBeApiSchemaList) {

        for (BeApiSchema item : todoHandleBeApiSchemaList) {

            for (Map.Entry<String, BeApiField> subItem : item.getFieldMap().entrySet()) {

                if (subItem.getValue() != null) {
                    continue;
                }

                BeApiSchema propertiesBeApiSchema = beApiSchemaMap.get(subItem.getKey());
     ...
     * 处理：所有的对象
     */
    private static void handleBeApiSchemaMap(JSONObject schemas, HashMap<String, BeApiSchema> beApiSchemaMap,
        List<BeApiSchema> todoHandleBeApiSchemaList) {
        ...
        for (Map.Entry<String, Object> item : schemas.entrySet()) {
            ...
            BeApiSchema apiSchema = new BeApiSchema();
     ...
     * 处理：所有的对象，处理：字段
     */
    private static void handleBeApiSchemaMapProperties(HashMap<String, BeApiSchema> beApiSchemaMap,
        List<BeApiSchema> todoHandleBeApiSchemaList, BeApiSchema apiSchema, JSONObject properties) {

        HashMap<String, BeApiField> fieldMap = MapUtil.newHashMap();
        ...
        for (Map.Entry<String, Object> item : properties.entrySet()) {
            ...
            if (StrUtil.isBlank(refStr)) { // 为空则表示是：一般类型
                ...
                BeApiSchema propertiesBeApiSchema = beApiSchemaMap.get(beApiSchemaMapKey); // 看这个对象是否存在于 map中
